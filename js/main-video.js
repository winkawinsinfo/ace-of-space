/* ============================================================
   ...
   timeline (scene progress 0 → 1, scene = 360vh → 260vh of travel):
     ...
     0.10–0.48  principles — combined single screen (fade in → hold → fade
                out), replacing the earlier 3-slide cycling treatment
...
(function () {
  ...
  if (scene && video && !reduced) {
    ...
    var principlesBlock = scene.querySelector('.layer-principles');
    ...
    function update() {
      ...
      /* --- principles: single combined block — fade in, hold, fade out --- */
      if (principlesBlock) {
        var pFadeIn  = ease(seg(p, 0.10, 0.16));
        var pFadeOut = 1 - ease(seg(p, 0.42, 0.48));
